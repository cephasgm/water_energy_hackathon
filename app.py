...
# Mock user database (in memory – replace with real DB later)
users = {}  # email -> {name, password_hash}

def hash_password(pwd):
    return hashlib.sha256(pwd.encode()).hexdigest()

# Simulation loop (same as before, but keep it running)
def simulation_loop():
...
def dashboard():
    ...
    # Render your original dashboard (you can reuse the HTML from previous answer)
    # For simplicity, I'll return the same template but with live data.
    ...
    try:
        ...
        pass
    # Reuse the same HTML template from previous answer but now protected
    dashboard_html = """
    <!DOCTYPE html>
    <html>
    <head><title>Dashboard | Hack the Watts</title><meta http-equiv="refresh" content="5">
    <style>/* same as earlier dashboard styles */</style>
    </head>
    <body>... (copy the full dashboard HTML from previous answer) ...</body>
    </html>
    """
    # For brevity, I assume you'll copy the dashboard HTML here.
    # But to avoid duplication, I'll provide a separate file `dashboard.html` in templates.
    return render_template('dashboard.html', level=level, level_percent=level_percent,
                           pump_running=pump_running, energy_kwh=energy_kwh, water_m3=water_m3,
                           energy_per_m3=energy_per_m3, cost_tzs=cost_tzs,
                           decision_reason=last_reason, leak=sensor.leak_active,
                           efficiency=efficiency, table_data=table_data)
